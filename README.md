...
`deno install --unstable [permissions] -n mgraph https://deno.land/x/microsoft_graph_cli@v1.1.1/cli.ts`
...
- (Optional) You may install any [released version](https://github.com/nakleiderer/microsoft-graph-cli/releases) of this CLI by changing `v1.1.1` to the desired version.
...
`deno run --unstable [permissions] https://deno.land/x/microsoft_graph_cli@v1.1.1/cli.ts`

- Replace `[permissions]` in this command with any permissions you wish to grant to the CLI. See [Permissions](#Permissions) for all optional and required permissions and their rationale. Example: `--allow-net --allow-read --allow-write`
- (Optional) If you would like to grant all permissions to this CLI, you may replace `[permissions]` with `--allow-all`. This will prevent errors if future versions require new permissions, but is considered less secure.
- (Optional) You may use any [released version](https://github.com/nakleiderer/microsoft-graph-cli/releases) of this CLI by changing the `v1.1.1` to the desired version.
...
`deno install --unstable [permissions] -n mgraph -f https://deno.land/x/microsoft_graph_cli@v1.1.1/cli.ts`

- Replace `[permissions]` in this command with any permissions you wish to grant to the CLI. See [Permissions](#Permissions) for all optional and required permissions and their rationale. Example: `--allow-net --allow-read --allow-write`
- (Optional) If you would like to grant all permissions to this CLI, you may replace `[permissions]` with `--allow-all`. This will prevent errors if future versions require new permissions, but is considered less secure.
- (Optional) You may install this cli with a different executable name by replace `mgraph` with a name of your choice. If you do so, make sure to replace `mgraph` with your selected name in all usage examples.
- (Optional) You may install any [released version](https://github.com/nakleiderer/microsoft-graph-cli/releases) of this CLI by changing the `v1.1.1` to the desired version.

If you use the [deno run](#use-deno-install-preferred) installation option, simply change the `v1.1.1` to the [latest release](https://github.com/nakleiderer/microsoft-graph-cli/releases/latest) when you next run the program.
